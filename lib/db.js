...
// Change listHotels to accept an optional parameter
async function listHotels(includeHidden = false) {
  ...
  return (data || [])
    .filter(row => includeHidden || !row.data || row.data.hidden !== true)
    .map(row => ({
      id: row.id,
      name: row.name,
      location: row.location,
      created_at: row.created_at,
      hidden: Boolean(row.data && row.data.hidden)
    }));
}

// Add the restore helper
async function unhideHotel(id) {
...
// Update the exports at the bottom of lib/db.js to include unhideHotel:
module.exports = { listHotels, getHotel, saveHotel, createHotel, hideHotel, unhideHotel, deleteHotel };
