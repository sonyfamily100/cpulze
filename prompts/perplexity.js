// Perplexity — Full Coverage (Part A / B / C), self-contained.
// Uses Perplexity's OpenAI-compatible chat/completions API. Sonar models
// are online/search-grounded by default. `citations` in the response is
// a useful proxy for grounding but not a guarantee — a thin or irrelevant
// citation list (e.g. an unrelated organization sharing a similar name)
// still warrants a manual look.
...
const SCORECARD_INSTRUCTIONS =
  ...
  'MANDATORY GROUNDING RULE: base every claim strictly on real, current web search results. Do not rely on ' +
  'training data. If a field is not confidently known, OMIT IT rather than inventing content. Fabricating a ' +
  'quote, reviewer, date, or platform is a critical failure even if the underlying theme is real.';
...
const NARRATIVE_INSTRUCTIONS =
  ...
  'MANDATORY GROUNDING RULE: base every claim strictly on real, current web search results for each of the ' +
  'three queries below. Do not rely on training data or general knowledge about hotels.';
...
function buildPartA(hotelName, location) {
  ...
  return { model: 'sonar-pro', messages: [{ role: 'system', content: SCORECARD_INSTRUCTIONS }, { role: 'user', content: userText }], max_tokens: 6000 };
}
...
function buildPartC(hotelName, location) {
  ...
  return { model: 'sonar-pro', messages: [{ role: 'system', content: NARRATIVE_INSTRUCTIONS }, { role: 'user', content: userText }], max_tokens: 3000 };
}

// Call: POST https://api.perplexity.ai/chat/completions, Authorization: Bearer PERPLEXITY_API_KEY
