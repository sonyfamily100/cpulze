// Gemini — Full Coverage (Part A / B / C), self-contained.
// Uses Gemini's generateContent REST API. No tool_choice:"required"
// equivalent exists — ALWAYS check response.candidates[0].groundingMetadata
// after the call. An empty {} or missing webSearchQueries means search
// never fired, regardless of how confident or complete the text looks —
// treat that response as unverified/likely fabricated, not just uncertain.
...
const SCORECARD_INSTRUCTIONS =
  ...
  'MANDATORY GROUNDING RULE: you must actually use Google Search grounding. Do not rely on training data. Every ' +
  ...
  'failure even if the underlying theme is real. If grounded search results are not available, output only ' +
...
const NARRATIVE_INSTRUCTIONS =
  ...
  'MANDATORY GROUNDING RULE: use Google Search grounding for each of the three queries. Do not rely on training ' +
  'data. If grounded search results are not available, output only GROUNDING_FAILED: true and stop.';

function wrap(instructions, userText, maxTokens) {
  return {
...
function buildPartA(hotelName, location) {
  ...
  return wrap(SCORECARD_INSTRUCTIONS, userText, 6000);
}
...
function buildPartC(hotelName, location) {
  ...
  return wrap(NARRATIVE_INSTRUCTIONS, userText, 3000);
}

// Call: POST https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-pro:generateContent?key=GEMINI_API_KEY
module.exports = { buildPartA, buildPartB, buildPartC, model: 'gemini-2.5-pro' };